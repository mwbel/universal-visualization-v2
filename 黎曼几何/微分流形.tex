...
\subsubsection*{核心概念拆解}
\paragraph*{A. $M$ 是一个 Hausdorff 空间}
...
\paragraph*{B. 局部同胚性 (Local Homeomorphism)}
...
\paragraph*{C. 坐标卡 (Coordinate Chart)}
...
设  $M$  是一个  $m$  维拓扑流形,  $\mathcal{A}$  是  $M$  的一个  $C^r$  微分结构, 则称  $(M, \mathcal{A})$  是一个  $m$  维  $C^r$  微分流形. 此时,  $\mathcal{A}$  中的坐标卡称为  $C^r$  微分流形  $(M, \mathcal{A})$  的容许坐标卡.
...
\begin{myremark}
这一部分内容探讨了流形上不同坐标系之间如何“对话”。 Jacobi 矩阵是连接不同局部观察者的桥梁。
...
\begin{myremark}
\subsection*{例 1.2 的直观解读：代数对象的几何化}
...
$$
(c为常数)上恒不为零，则  $M_{c}$  是一个  $_{n}$  维的  $C^r$  微分流形。

\begin{quote}
\textbf{\textcolor{blue}{直观解读：}}
隐函数定理告诉我们：如果一个方程 $f(x)=c$ 的梯度不为零，那么它的解集在局部上看起来就像是被“压平”后的欧氏空间。比如球面 $S^n$。
\end{quote}
...
\textbf{例1.4} 设 $f$ 是定义在 $\mathbb{R}^{n+1}$ 上的实值 $C^r$ 函数，如果 $\text{grad} f \neq 0$ 在水平集 $M_c = \{f=c\}$ 上成立，则 $M_c$ 是 $n$ 维微分流形。
