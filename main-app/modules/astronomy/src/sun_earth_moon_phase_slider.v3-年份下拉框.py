...
# 载入月球表面纹理（灰度或彩色均可）
moon_texture = Image.open("src/moon_texture.jpg").convert("L")  # 灰度
moon_texture = moon_texture.resize((MOON_DISC_RES, MOON_DISC_RES))
...
def make_moon_phase_image(t, N=MOON_DISC_RES):
    ...
    # === 加入月球纹理 ===
    tex = moon_texture[:N, :N]  # 裁剪到相同大小
    texI = np.full_like(X, np.nan, dtype=float)
    texI[mask] = tex[mask] * I[mask]  # 纹理与光照叠加

    # 关键：圆外设为 None，这样 Plotly 会完全透明
    texI[~mask] = None
...
x_lin, y_lin, texI0 = make_moon_phase_image(ts_list[idx0])
...
fig.add_trace(go.Heatmap(
    ...
    showscale=False,
    hoverinfo="skip",   # 不显示 hover
    # zauto=False,        # 固定颜色范围，避免自动拉伸
...
for i, t in enumerate(ts_list):
    ...
    _, _, texI = make_moon_phase_image(t, N=MOON_DISC_RES)
    ...
    frame = go.Frame(
        ...
        data=[
            ...
            go.Scatter3d(x=[mx], y=[my], z=[mz],
                         ...
                         name="月亮"),
            go.Heatmap(z=texI, x=x_lin, y=y_lin, zmin=0, zmax=1,
