...
fig.add_trace(go.Scatter3d(
    ...
    mode="lines", line=dict(width=1, color="rgba(255,255,255,0.8)"), # 加粗+白色
...
fig.add_trace(go.Scatter3d(
    x=[ex0], y=[ey0], z=[ez0],
    mode="markers", marker=dict(size=6, color="deepskyblue"), # 地球稍大
    name="地球"
), row=1, col=1)
...
for i, t in enumerate(ts_list):
    ...
    mx, my, mz = ex + mgx, ey + mgy, ez + mgz   # 平移到地球位置
    ...
    frame = go.Frame(
        ...
        data=[
            go.Scatter3d(x=[ex], y=[ey], z=[ez],
                         mode="markers", marker=dict(size=6, color="deepskyblue"),
                         name="地球"),
...
fig.update_layout(
    scene=dict(
        ...
        bgcolor="black", camera=dict(eye=dict(x=1.6, y=1.6, z=0.9))
    ),
    ...
    sliders=[{
        "active": 0, "x": 0.25, "y": -0.08, "len": 0.6,
