...
def draw_scene(fig, latitude, radius=DEFAULT_RADIUS, zenith_extend: float = 1.5):
    ...
    for lat in lats:
        ...
        # 选择更亮的颜色，赤道用特殊颜色
        if abs(lat) < 1:  # 赤道
            line_color = "rgba(255, 100, 100, 1.0)"
            line_width = 3
        else:
            line_color = "rgba(150, 150, 255, 1.0)"
            line_width = 2
    ...
    for lon in lons:
        ...
        # 选择更亮的颜色，本初子午线用特殊颜色
        if abs(lon) < 1:  # 本初子午线
            line_color = "rgba(100, 255, 100, 1.0)"
            line_width = 3
        else:
            line_color = "rgba(150, 150, 255, 1.0)"
            line_width = 2
    ...
    # 天顶/天底与铅垂线（超出球面一些）
    # 天顶 (alt=90)
    zenith_x, zenith_y, zenith_z = _spherical_to_cartesian(
        0, 90, radius * zenith_extend
    )
    fig.add_trace(
        go.Scatter3d(
            x=[zenith_x],
...
def compute_body_point_for_date(
    body,
    observer,
    ts,
    local_date,
    ...
    local_dt = tz.localize(
        datetime(local_date.year, local_date.month, local_date.day, hour, minute, 0)
    )
    ...
    if ephemeris is not None and str(body_key).lower() == "moon":
        ...
        # 简易月相标签
        if 0 <= phase_angle_deg < 45:
            moon_phase = "新月"
        elif 45 <= phase_angle_deg < 90:
            moon_phase = "娥眉月"
        elif 90 <= phase_angle_deg < 135:
            moon_phase = "上弦月"
        elif 135 <= phase_angle_deg < 180:
            moon_phase = "盈凸月"
        elif 180 <= phase_angle_deg < 225:
            moon_phase = "满月"
        elif 225 <= phase_angle_deg < 270:
            moon_phase = "亏凸月"
        elif 270 <= phase_angle_deg < 315:
            moon_phase = "下弦月"
        else:
            moon_phase = "残月"
...
def main():
    parser = argparse.ArgumentParser(
        description="交互式 3D 天球模型，展示太阳与月亮位置。"
    )
    parser.add_argument(
        "--lat",
        type=float,
    ...
    parser.add_argument(
        "--year", type=int, default=DEFAULT_YEAR, help=f"年份 (默认: {DEFAULT_YEAR})"
    )
    parser.add_argument(
        "--hour",
        type=int,
    ...
    try:
        ...
        print(
            f"错误: 未知的时区 '{tz_name}'. 请使用有效的 IANA 时区名称 (例如 'Asia/Shanghai')."
        )
        return
    ...
    sun_annual_idx = add_sun_noon_annual_track(
        ...
        tz_name,
        hour=fixed_hour,
        minute=fixed_minute,  # 你现在就是 12:00；也可固定写 hour=12, minute=0
        color="orange",
    )
            ...
            buttons=list(
                [
                    dict(
                        ...
                        args=[
                            {
                                "visible": [True] * num_static_traces
                                + [True for _ in range(len(all_dates) * 2)]
                            },  # 所有太阳和月亮 trace 都可见
                            {"showlegend": True},
                        ],
                    ),
                        ...
                        args=[
                            {
                                ...
                                + [
                                    ...
                                    for j in range(len(all_dates) * 2)
                                ]
                            },  # 仅太阳 trace 可见
                            {"showlegend": True},
                        ],
...
def create_celestial_sphere_slider_plot(latitude=31.23, longitude=121.47, timezone="Asia/Shanghai",
                                        year=2025, hour=12, minute=0, date_index=0):
    """
    为API创建天球滑块图表的函数
    返回JSON格式的Plotly图表数据
    """
    try:
        ...
        # 加载星历
        ts = load.timescale()
        eph_path = os.path.join(os.path.dirname(__file__), 'de421.bsp')
        ...
        # 确保日期索引在有效范围内
        date_index = max(0, min(date_index, len(all_dates) - 1))
        ...
        # 添加太阳周年轨迹
        add_sun_noon_annual_track(
            fig, ephemeris, observer, ts, all_dates, timezone,
            hour=hour, minute=minute, color="orange"
        )

        # 计算当前日期的太阳位置
        sun_x, sun_y, sun_z, sun_hover_text = compute_body_point_for_date(
            sun, observer, ts, current_date, timezone, hour, minute,
        ...
        # 计算当前日期的月亮位置
        moon_x, moon_y, moon_z, moon_hover_text = compute_body_point_for_date(
            moon, observer, ts, current_date, timezone, hour, minute,
        ...
        fig.add_trace(go.Scatter3d(
            ...
            mode="markers",
            marker=dict(color="lightgray", size=8, symbol="circle"),
            name=f"月亮 ({hour:02d}:{minute:02d})",
            hoverinfo="text",
            hovertext=moon_hover_text
        ))
        ...
        # 设置布局
        max_extend = max(PLANE_EXTEND, LABEL_EXTEND, ZENITH_EXTEND)
        L = DEFAULT_RADIUS * max_extend * 1.02

        fig.update_layout(
            title_text=f"天球滑块视运动系统<br>"
                      f"观测点: 纬度 {latitude:.1f}°, 经度 {longitude:.1f}° ({timezone})<br>"
                      f"日期: {current_date.strftime('%Y-%m-%d')} ({hour:02d}:{minute:02d})",
            title_x=0.5,
            scene=dict(
                xaxis=dict(range=[-L, L], showbackground=False, showticklabels=False, zeroline=False, showgrid=False),
                yaxis=dict(range=[-L, L], showbackground=False, showticklabels=False, zeroline=False, showgrid=False),
                zaxis=dict(range=[-L, L], showbackground=False, showticklabels=False, zeroline=False, showgrid=False),
                aspectmode="cube",
                bgcolor="rgb(10,10,20)",  # 深蓝灰色背景，不是纯黑
                camera=dict(
                    eye=dict(x=1.5, y=1.5, z=1.2)  # 调整视角让线条更清楚
                )
            ),
            template="plotly_dark",
            showlegend=True,
            legend=dict(x=0.0, y=0.9, bgcolor="rgba(10,10,20,0.8)", bordercolor="rgba(255,255,255,0.3)", font=dict(color="white")),
        ...
        print(f"创建天球滑块图表失败: {e}")
        # 返回错误图表
        fig = go.Figure()
        fig.add_trace(go.Scatter3d(
            x=[0], y=[0], z=[0],
            mode="text",
            text=[f"天球滑块系统错误<br>{str(e)}"],
