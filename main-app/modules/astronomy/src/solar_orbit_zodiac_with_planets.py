...
def plot_solar_orbit_zodiac(date=None):
    ...
    # 七大行星轨道 (简化，以AU为半径)
    planets_data = {
        '水星': {'radius': 0.39, 'color': 'gray'},
        '金星': {'radius': 0.72, 'color': 'orange'},
        '火星': {'radius': 1.52, 'color': 'brown'},
        '木星': {'radius': 5.20 * 0.5, 'color': 'peru'}, # 缩放木星
        '土星': {'radius': 9.58 * 0.5, 'color': 'goldenrod'}, # 缩放土星
        '天王星': {'radius': 19.22 * 0.5, 'color': 'lightseagreen'}, # 缩放天王星
        '海王星': {'radius': 30.05 * 0.5, 'color': 'royalblue'} # 缩放海王星
    }
    ...
    for planet_name, data in planets_data.items():
        ...
        planet_orbit_x = radius * np.cos(theta)
        planet_orbit_y = radius * np.sin(theta)
        planet_orbit_z = np.zeros_like(theta)
        ax.plot(planet_orbit_x, planet_orbit_y, planet_orbit_z, color=color, linestyle=':', linewidth=0.5, label=f'{planet_name}轨道')
        
        # 标注行星名称 (只在轨道上标注一次)
        ax.text(radius * np.cos(np.deg2rad(45)), radius * np.sin(np.deg2rad(45)), 0, planet_name, color=color, fontsize=12, ha='center', va='center')
