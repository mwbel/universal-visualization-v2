...
def plot_body_annual_motion(fig, body, body_name,
                            color_day, color_night,
                            observer, location_topos, ts,
                            year: int,
                            tz_name='Asia/Shanghai',
                            ...
                            hour_step: float = 1.0):
    """
    向量化版：绘制全年周年视运动（逐日轨迹叠加）
    - 将全年所有“整点（可设步长）时刻”一次性批量送入 skyfield 计算 alt/az
    - 用 NaN 在每天结尾插针，保证每日为独立折线段
    - hour_step: 采样步长（小时），默认1小时；可设0.5以更平滑
    """
    from math import ceil
    local_tz = ZoneInfo(tz_name)

    # 1) 生成全年每一天的 [0, 24] 小时网格（可设步长）
    hours = np.arange(0.0, 24.0 + 1e-9, hour_step)  # 包含24点
    # 收集全年所有本地时刻（列表里每个元素是一整天的若干时刻）
    days_local = []
    n_days = 0
    for month in range(1, 13):
        for day in range(1, 32):
            try:
                local_midnight = datetime(year, month, day, 0, 0, tzinfo=local_tz)
            except ValueError:
                continue  # 跳过无效日期
            n_days += 1
            day_times = [local_midnight + timedelta(hours=float(h)) for h in hours]
            days_local.append(day_times)

    # 2) 扁平化为一个巨大的时刻序列，并插入“日分隔”占位，之后再映射回 NaN
    #    这里先不插NaN，先把真时刻统一转换为UTC并批量计算
    flat_local = [dt for day_list in days_local for dt in day_list]
    flat_utc = [dt.astimezone(ZoneInfo('UTC')) for dt in flat_local]

    years  = [dt.year  for dt in flat_utc]
    months = [dt.month for dt in flat_utc]
    days   = [dt.day   for dt in flat_utc]
    fracs  = [dt.hour + dt.minute/60 + dt.second/3600 + dt.microsecond/3.6e9 for dt in flat_utc]

    ts_all = ts.utc(years, months, days, fracs)

    # 3) 一次性批量计算全年所有时刻的 alt/az
    app = observer.at(ts_all).observe(body).apparent()
    alt, az, _ = app.altaz()
    alt_deg = alt.degrees
    az_deg  = az.degrees

    # 4) 转直角坐标
    x_all, y_all, z_all = _spherical_to_cartesian(az_deg, alt_deg)

    # 5) 在“每天的最后一个点”之后插 NaN 断开连线
    #    每天有 len(hours) 个点；我们为 Plotly 构造带 NaN 的数组
    seg = int(len(hours))
    xn, yn, zn = [], [], []
    for i in range(n_days):
        s = i * seg
        e = s + seg
        xn.extend(x_all[s:e]); yn.extend(y_all[s:e]); zn.extend(z_all[s:e])
        # 在每一天末尾插入一个 NaN 作为断点
        xn.append(np.nan); yn.append(np.nan); zn.append(np.nan)

    # 主轨迹（用日间色；夜间覆盖另加一层虚线）
    fig.add_trace(go.Scatter3d(
        x=xn, y=yn, z=zn,
        mode="lines",
        line=dict(color=color_day, width=1),
        name=f"{body_name}周年轨迹",
        hoverinfo="none",
        showlegend=True,
        legendgroup=f"{body_name}-annual"
    ))

    # 6) 夜间覆盖：高度<0 的点保留，≥0 的点置 NaN；注意保持与插针后的同形状
    alt_full = np.array(list(alt_deg) + [np.nan]*n_days)  # 与 xn 对齐：每日日末多了一个 NaN
    night_mask = alt_full < 0
    night_x = np.where(night_mask, xn, np.nan)
    night_y = np.where(night_mask, yn, np.nan)
    night_z = np.where(night_mask, zn, np.nan)
    fig.add_trace(go.Scatter3d(
        x=night_x, y=night_y, z=night_z,
        mode="lines",
        line=dict(color=color_night, width=1, dash="dash"),
        name=f"{body_name}周年(夜间)",
        hoverinfo="none",
        showlegend=False,
        legendgroup=f"{body_name}-annual"
    ))
    ...
    if show_realtime:
        ...
        t_now = ts.utc(now_utc.year, now_utc.month, now_utc.day,
                       now_utc.hour + now_utc.minute/60 + now_utc.second/3600 + now_utc.microsecond/3.6e9)
        alt_now, az_now, _ = observer.at(t_now).observe(body).apparent().altaz()
        rx, ry, rz = _spherical_to_cartesian(az_now.degrees, alt_now.degrees)
        fig.add_trace(go.Scatter3d(
            x=[_tofloat(rx)], y=[_tofloat(ry)], z=[_tofloat(rz)],
            mode="markers+text",
            marker=dict(size=6, color=color_day, symbol="circle-open"),
            text=[f"{body_name}(实时 {now_local.strftime('%m-%d %H:%M')})"],
            textfont=dict(color="red", size=9),
            textposition="top center",
            showlegend=False
        ))
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            camera=dict(eye=dict(   # 初始相机位置（球面坐标转直角的小技巧）
                x=1.5*np.cos(np.deg2rad(20))*np.cos(np.deg2rad(30)),
                y=1.5 *np.cos(np.deg2rad(20))*np.sin(np.deg2rad(30)),
