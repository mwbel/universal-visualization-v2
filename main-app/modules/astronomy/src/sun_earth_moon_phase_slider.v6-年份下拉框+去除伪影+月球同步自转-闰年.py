...
def calc_phase_name(t):
    """根据太阳-地球-月亮的相对角度，返回中文月相名称"""
    e2m = moon_geo(t)     # Earth->Moon
    e2s = sun.at(t).observe(earth).position.au   # Earth->Sun
    # 相位角 (太阳-地球-月亮)
    cos_phi = np.dot(e2m, e2s) / (np.linalg.norm(e2m) * np.linalg.norm(e2s))
    phi = np.arccos(np.clip(cos_phi, -1, 1))   # 0~pi
    ...
    if illum < 0.03:
        ...
        return "娥眉月"
    elif abs(illum - 0.5) < 0.03 and np.cross(e2s, e2m)[2] > 0:
        return "上弦月"
    elif 0.5 < illum < 0.97 and np.dot(e2s, e2m) < 0:
        return "盈凸月"
    elif illum >= 0.97:
        return "满月"
    elif 0.5 < illum < 0.97 and np.dot(e2s, e2m) > 0:
        ...
        layout=go.Layout(   # 每帧更新月相名称
            annotations=[
                dict(
                    ...
                    x=0.85, y=0.85, # 0.85 表示右侧子图靠右；1.05 在子图上方
