...
def _spherical_to_cartesian(azimuth_deg, altitude_deg, radius: float = 1.0):
    ...
    """
    az = np.deg2rad(
        np.atleast_1d(azimuth_deg)
    )  # 方位角（度→弧度），0°=北，90°=东（skyfield返回亦如此）
    alt = np.deg2rad(
        np.atleast_1d(altitude_deg)
    )  # 高度角（度→弧度），0°=地平，90°=天顶
...
def plot_body_daily_motion(
    ...
    if show_rise_set:
        ...
        t0_utc = ts.utc(
            local_midnight.astimezone(ZoneInfo("UTC")).year,
            local_midnight.astimezone(ZoneInfo("UTC")).month,
            local_midnight.astimezone(ZoneInfo("UTC")).day,
            0,
        )
        t1_utc = ts.utc(
            next_midnight.astimezone(ZoneInfo("UTC")).year,
            next_midnight.astimezone(ZoneInfo("UTC")).month,
            next_midnight.astimezone(ZoneInfo("UTC")).day,
            0,
        )
        ...
        for t_e, ev in zip(t_events, events):
            ...
            rs_x.append(_tofloat(ex))
            rs_y.append(_tofloat(ey))
            rs_z.append(_tofloat(ez))
...
def plot_geocentric_celestial_sphere(
    ...
    local_midnight = datetime(
        local_date.year, local_date.month, local_date.day, 0, 0, tzinfo=local_tz
    )
    ...
    latitude_text = (
        f"北纬 {latitude:.1f}°" if latitude >= 0 else f"南纬 {-latitude:.1f}°"
    )
    ...
    # 保存为HTML文件（以便在网页中集成）
    # 注意：为了不覆盖已增强的手写交互版页面，这里修改了输出文件名
    output_path = "../original/celestial_sphere_7planets_python_generated.html"
